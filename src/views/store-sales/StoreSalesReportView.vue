    ...
    <div class="bg-white rounded-xl border border-gray-200">
      ...
      >
        <Column field="name" header="Tienda" sortable>
          <template #body="{ data }">
            <div class="font-medium text-gray-900">{{ data.name || '#' + data.id }}</div>
...
function applyRange() {
  ...
  store.fetchReport(toISODate(dateFrom.value), toISODate(dateTo.value))
}
...
function exportCsv() {
  const headers = ['Tienda', 'Slug', 'Transacciones', 'Monto total', 'Ticket promedio', 'Ticket más alto', 'Ticket más bajo']
  const escape = (v: string | number) => `"${String(v).replace(/"/g, '""')}"`
  const lines = rows.value.map(r =>
    [r.name, r.slug, r.transactions, r.total_amount, r.avg_ticket, r.highest_ticket, r.lowest_ticket]
      .map(escape)
      .join(',')
  )
